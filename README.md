...
|------|------|---------|------|---------|---------|--------|
| 贝壳 | ke | 动态读取档位+点击链接 | 有，翻页 | 详情页有 | 详情页有 | 必须点 |
| 安居客 | ajk | 动态读取档位+点击链接 | 无，滚动到底 | **无**（挂牌均价顶替） | 结果页社区卡片 | 不点 |
| 链家 | lj | 动态读取档位+点击链接 | 有，翻页 | 详情→成交列表翻页 | 不取 | 必须点 |
| 房天下 | fang | 动态读取档位+点击链接 | 有，翻页 | 详情→小区成交 tab | 不取 | Ctrl+点击 |
| 乐有家 | lyj | 动态读取档位+点击链接 | 有，翻页 | **无**（小区均价顶替） | 结果页社区信息卡 | 不点 |
...
4. 接收询价请求：`communityName`、`area`（精确面积）。
5. 刷新常驻页面，执行轻量保活。
6. 搜索目标小区。
7. 结果页按面积筛选。
