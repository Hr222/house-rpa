...
LUXURY_DEAL_MIN_SUPPORT = 3
LUXURY_DEAL_DIRECT_MIN_GAP = 0.10
LUXURY_DEAL_MIX_MAX_GAP = 0.30
LUXURY_DEAL_LISTING_WEIGHT = 0.30
LUXURY_DEAL_WEIGHT = 0.70
...
def _select_deal_price(
    ...
    """选择目标面积成交价，不应用挂牌折扣。"""
    candidate = _select_deal_candidate(
        deal_price_lists,
        max_relative_deviation=max_relative_deviation,
    )
    return candidate.quote_price if candidate is not None else None


def _select_deal_candidate(
    deal_price_lists: Iterable[Iterable[float]],
    max_relative_deviation: float = WEIGHTED_MEDIAN_MAX_RELATIVE_DEVIATION,
    ...
    if not candidates:
        return None
    return min(candidates, key=lambda candidate: candidate.quote_price)


def _combine_luxury_listing_and_deal(
    listing_peak: float,
    ...
    if (
        deal_candidate.count < LUXURY_DEAL_MIN_SUPPORT
        or gap < LUXURY_DEAL_DIRECT_MIN_GAP
        or gap > LUXURY_DEAL_MIX_MAX_GAP
    ):
        return (listing_peak + deal_peak) / 2
    if gap < 0.20:
        return deal_peak
    return (
        listing_peak * LUXURY_DEAL_LISTING_WEIGHT
        + deal_peak * LUXURY_DEAL_WEIGHT
    )
...
class WeightedMedianAlgorithm:
    ...
    def evaluate(self, inputs: AlgorithmInput) -> AlgorithmEvaluation:
        ...
        deal_candidate = _select_deal_candidate(inputs.deal_price_lists)
        deal_avg = deal_candidate.quote_price if deal_candidate is not None else None
        ...
        if decision.final_price is not None and deal_avg is not None:
            combined_price = (quote_avg + deal_avg) / 2
            if (
                inputs.area is not None
                and inputs.area >= LUXURY_TRANSITION_START_AREA
                and deal_candidate is not None
                and quote_avg is not None
            ):
                combined_price = _combine_luxury_listing_and_deal(
                    quote_avg,
                    deal_candidate,
                )
            decision = Decision(
