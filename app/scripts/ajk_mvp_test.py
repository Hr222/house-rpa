...
用法：
  python -m app.scripts.ajk_mvp_test --manual-login --excel
"""
    ...
    *,
    ...
    print_mvp_result(
        ...
        deals={
            ...
            "records": [],
            "substitute": (
                f"挂牌均价顶替 {listing_price}元/㎡（加权落点中位数算法不使用）"
            ),
        },
        result={
...
async def main(
    ...
    try:
        ...
        if not result_blocked:
            ...
            log.info(
                "[4] 在售均价=%.2f 挂牌均价(顶替成交)=%s 算法=加权落点中位数 最终价=%.2f 分支=%s",
                listing_avg,
                listing_price,
                final_price,
                branch,
