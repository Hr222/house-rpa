...
用法：
  python -m scripts.initialize_community_page.community_page.lj_community_page_mvp --manual-login \
...
async def collect_deal_page_url(
    page,
    *,
    start_url: str,
    expected_host: str,
    ...
    链家成交页与挂牌页共用 xiaoqu ID（/chengjiao/c{id}/，与 adapter 成交流程
    新标签页的 /chengjiao/ 路径同源）；零成交小区页面合法为空，这里只校验
    URL 归属，不校验成交条数。
    """
    origin = f"{urlparse(start_url).scheme}://{expected_host}"
    deal_page_url = f"{origin}/chengjiao/c{xiaoqu_id}/"
    ...
    actual_id = re.search(r"/chengjiao/c(\d+)/", urlparse(actual_url).path)
    if not actual_id or actual_id.group(1) != xiaoqu_id:
        raise RuntimeError(
            f"成交页 xiaoqu ID 校验失败：期望 {xiaoqu_id}，实际 URL={actual_url}"
        )
    return actual_url
...
async def main(
    ...
    （链家成交页与挂牌页共用 xiaoqu ID），供统一初始化入口复用。
    """
    if debug:
        set_debug_mode(True)
