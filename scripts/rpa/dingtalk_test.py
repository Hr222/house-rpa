...
用法::
    ...
    python -m scripts.rpa.dingtalk_test

    # 方式2：命令行传参（优先于环境变量）
    python -m scripts.rpa.dingtalk_test https://oapi.dingtalk.com/robot/send?access_token=xxx

    # 方式3：用 uvicorn 同一环境运行
    python -m scripts.rpa.dingtalk_test --url https://oapi.dingtalk.com/robot/send?access_token=xxx
...
from app.rpa.utils.dingtalk import send_text, send_markdown
...
async def main(webhook_url: str | None = None):
    ...
    if not webhook:
        ...
        print("   方式2: python -m scripts.rpa.dingtalk_test <webhook_url>")
        return
